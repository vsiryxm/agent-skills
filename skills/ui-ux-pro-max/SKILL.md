...
A queryable design knowledge base that helps you make informed UI/UX decisions — style selection, color palettes, typography pairings, chart recommendations, animation presets, and stack-specific implementation guidance. Unlike rule-based skills that encode engineering discipline, this skill provides a searchable database of design options with reasoning, so you can ground visual decisions in evidence rather than guessing.
...
The search script lives inside this skill's own directory. Always invoke it by its full path — do not assume a particular working directory:
...
|---|---|---|
| "I'll just pick a style from memory" | AI models default to visually safe palettes (purple/indigo), making every app look identical. | Run `--design-system` with product type and keywords to get a grounded recommendation. |
| "I don't need a search for this" | Guessing at color/typography/animation without data produces inconsistent, ungrounded choices. | Use `--domain` for any non-trivial visual decision. Even a single query takes seconds. |
| "The search returned 0 results, I'll just make something up" | Fabricating output that looks like search results undermines trust in the skill. | Retry with a narrower query; if still empty, state it explicitly and fall back to the priority table in SKILL.md. |
| "I'll skip the stack query, it's all the same" | Each stack has distinct patterns (React memo vs Vue computed, Next.js Suspense vs Svelte await). | Pass the detected stack explicitly with `--stack`. |
| "Emoji icons are fine for now" | Emojis are font-dependent, inconsistent across platforms, and cannot be controlled via design tokens. | Use vector icons (Lucide, Phosphor). See `references/pro-rules.md`. |
| "I'll regenerate the design system without reading the existing MASTER.md" | Prior design decisions may have been made deliberately and should not be silently discarded. | Read existing `MASTER.md` first; only use `--force` with explicit user authorization. |

## Red Flags

- Applying search results without verifying the returned domain/category fits the user's product
- Presenting a 0-result search as if it returned data
- Using emoji as structural icons (navigation, settings, system controls)
- Guessing a stack instead of detecting it from `package.json` / project files
- Passing the same query through multiple unrelated domains hoping something sticks
- Regenerating a `MASTER.md` with `--force` without reading the existing one first
- Skipping `references/pro-rules.md` before delivering native/mobile app UI
- Hardcoding a stack's patterns into a different stack (e.g., React memo in Vue)

## Verification

After using this skill for a UI/UX task:

- [ ] The detected stack matches the project's actual framework (check `package.json` / project files)
- [ ] Search results were verified: domain/category and top result identity match the user's request
- [ ] 0-result searches were handled honestly (retry → fallback → explicit statement)
- [ ] For design system generation: the output includes style, colors, typography, effects, and anti-patterns
- [ ] For persisted design systems: `MASTER.md` exists and was read before any `--force` regeneration
- [ ] For native/mobile UI: `references/pro-rules.md` Pre-Delivery Checklist was reviewed
- [ ] No private project data was included in search queries or persisted output
